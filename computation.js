...
const pageName2 = document.getElementById("computationData");
...
addTextBtn.addEventListener("click", () => {
  const box = createTextBox("", "50px", "50px");
...
// Add text box on any click inside the editLayer
editLayer.addEventListener("dblclick", (e) => {
  // Prevent adding a text box if click was on an existing text box
  ...
  const box = createTextBox("", `${top}px`, `${left}px`);
  editLayer.appendChild(box);
  makeDraggable(box);
  box.focus();
  saveToLocal();
...
function saveToLocal() {
  ...
  localStorage.setItem("pageName2", JSON.stringify(data));
}

function loadFromLocal() {
  const savedData = localStorage.getItem("pageName2");
  if (savedData) {
